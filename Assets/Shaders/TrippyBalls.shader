...
//                float y = sin(saturate(i.uv.y * _SinTime.x * _CosTime.y) + 1.2);  
                ...
                float x = saturate(sin(i.uv.x)) + 0.7; 
                float y = saturate(sin(i.uv.y)) + 0.7;
                fixed4 col = tex2D(_MainTex, float2(x,y) * i.uv.xy * i.uv.yx);// * tex2D(_MainTex, 1 / i.uv.xy) / tex2D(_MainTex, -i.uv.xy);
